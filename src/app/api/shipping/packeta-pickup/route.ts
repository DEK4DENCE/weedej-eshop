...
// Fallback dataset — shown when Packeta API is unavailable
...
async function tryFetch(url: string): Promise<PacketaPoint[] | null> {
  try {
    ...
    if (!res.ok) { console.error("[PacketaPickup] HTTP", res.status, url); return null }
    const ct = res.headers.get("content-type") ?? ""
    if (!ct.includes("json")) { console.error("[PacketaPickup] non-JSON response from", url); return null }
    const json = await res.json()
    const raw: PacketaPoint[] = Array.isArray(json) ? json : (json.data ?? json.pickupPoints ?? json.branches ?? [])
    return raw.length ? raw : null
  } catch (e: any) {
    console.error("[PacketaPickup] fetch error", url, e?.message)
    return null
  }
}
...
async function fetchPoints(): Promise<PacketaPoint[]> {
  ...
  // Try different Packeta API endpoint formats in order
  const urls = [
    ...
    `https://www.zasilkovna.cz/api/v4/pickup-points?apiKey=${key}&lang=cs&country=cz`,
  ...
  console.error("[PacketaPickup] all endpoints failed — using fallback data")
  return FALLBACK
}
