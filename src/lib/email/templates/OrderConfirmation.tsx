import {
  Html,
  Head,
  Body,
  Container,
  Text,
  Button,
  Link,
  Hr,
  Preview,
  Row,
  Column,
  Section,
} from '@react-email/components'

interface OrderItem {
  productName:  string
  variantLabel: string
  quantity:     number
  unitPrice:    number // haléře (cents)
}

interface ShippingAddress {
  fullName:   string
  line1:      string
  city:       string
  postalCode: string
  country:    string
}

// ─── Props ────────────────────────────────────────────────────────────────────

interface OrderConfirmationWithInvoiceProps {
  name:              string
  orderNumber:       string   // ESH{YYYY}{XXXX}
  items:             OrderItem[]
  subtotalAmount:    number   // haléře
  shippingAmount:    number   // haléře
  totalAmount:       number   // haléře
  deliveryType:      'COURIER' | 'PICKUP_IN_STORE'
  shippingAddress?:  ShippingAddress
  invoiceNumber?:    string   // VF{YYYY}{XXXX} — shown in email body
  invoiceAttached?:  boolean  // true = PDF is actually attached; false = available in account
}

interface OrderConfirmationAckProps {
  name:             string
  internalOrderId:  string   // shown as reference, not ERP number
  items:            OrderItem[]
  subtotalAmount:   number
  shippingAmount:   number
  totalAmount:      number
  deliveryType:     'COURIER' | 'PICKUP_IN_STORE'
  shippingAddress?: ShippingAddress
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? 'https://weedej.com'

function formatPrice(cents: number) {
  return `${Math.round(cents / 100).toLocaleString('cs-CZ')} Kč`
}

// ─── Shared layout ────────────────────────────────────────────────────────────

function ItemsTable({ items, subtotalAmount, shippingAmount, totalAmount }: {
  items: OrderItem[]
  subtotalAmount: number
  shippingAmount: number
  totalAmount:    number
}) {
  return (
    <>
      <Text style={sectionTitle}>Přehled objednávky</Text>
      {items.map((item, i) => (
        <Row key={i} style={itemRow}>
          <Column style={itemInfo}>
            <Text style={itemName}>{item.productName}</Text>
            <Text style={itemVariant}>{item.variantLabel} × {item.quantity}</Text>
          </Column>
          <Column style={itemPrice}>
            <Text style={itemPriceText}>{formatPrice(item.unitPrice * item.quantity)}</Text>
          </Column>
        </Row>
      ))}

      <Hr style={divider} />

      <Row style={totalRow}>
        <Column><Text style={totalLabel}>Mezisoučet</Text></Column>
        <Column><Text style={totalValue}>{formatPrice(subtotalAmount)}</Text></Column>
      </Row>
      <Row style={totalRow}>
        <Column><Text style={totalLabel}>Doprava</Text></Column>
        <Column><Text style={totalValue}>{shippingAmount === 0 ? 'Zdarma' : formatPrice(shippingAmount)}</Text></Column>
      </Row>
      <Row style={totalRow}>
        <Column><Text style={grandTotalLabel}>Celkem</Text></Column>
        <Column><Text style={grandTotalValue}>{formatPrice(totalAmount)}</Text></Column>
      </Row>

      <Hr style={divider} />
    </>
  )
}

function DeliveryBlock({ deliveryType, shippingAddress }: {
  deliveryType:     'COURIER' | 'PICKUP_IN_STORE'
  shippingAddress?: ShippingAddress
}) {
  return (
    <>
      <Text style={sectionTitle}>Doručení</Text>
      {deliveryType === 'PICKUP_IN_STORE' ? (
        <Text style={paragraph}>
          <strong style={{ color: '#1d1d1f' }}>Osobní odběr</strong> — Vaše objednávka bude
          připravena k vyzvednutí na naší prodejně. Kontaktujeme vás, jakmile bude připravena.
        </Text>
      ) : shippingAddress ? (
        <Text style={paragraph}>
          <strong style={{ color: '#1d1d1f' }}>Doručení kurýrem</strong>
          <br />
          {shippingAddress.fullName}<br />
          {shippingAddress.line1}<br />
          {shippingAddress.postalCode} {shippingAddress.city}<br />
          {shippingAddress.country}
        </Text>
      ) : (
        <Text style={paragraph}>Doručení kurýrem</Text>
      )}
    </>
  )
}

// ─── Template 1: With ERP order number + invoice note (§ 28 ZDPH) ─────────────

/**
 * Mail #1 (success path): ERP responded — we have ESH number and invoice.
 * Invoice PDF is sent as an email attachment (legally required per § 28 ZDPH).
 */
export function OrderConfirmationWithInvoice({
  name,
  orderNumber,
  items,
  subtotalAmount,
  shippingAmount,
  totalAmount,
  deliveryType,
  shippingAddress,
  invoiceNumber,
  invoiceAttached,
}: OrderConfirmationWithInvoiceProps) {
  return (
    <Html lang="cs">
      <Head />
      <Preview>Vaše objednávka {orderNumber} je potvrzena — faktura v příloze</Preview>
      <Body style={body}>
        <Container style={container}>
          <Text style={logo}>Weedej</Text>

          <Text style={heading}>Potvrzení objednávky</Text>
          <Text style={paragraph}>Dobrý den, {name},</Text>
          <Text style={paragraph}>
            Děkujeme za vaši objednávku! Přijali jsme vaši platbu a připravujeme zásilku.
            Jakmile bude objednávka odeslána, obdržíte další e-mail se sledovacím číslem.
          </Text>

          <Section style={orderBox}>
            <Text style={orderLabel}>Číslo objednávky (ERP)</Text>
            <Text style={orderNumberStyle}>{orderNumber}</Text>
            {invoiceNumber && (
              <>
                <Text style={{ ...orderLabel, marginTop: '10px' }}>Číslo faktury</Text>
                <Text style={{ ...orderNumberStyle, fontSize: '14px' }}>{invoiceNumber}</Text>
              </>
            )}
          </Section>

          {invoiceNumber && invoiceAttached && (
            <Text style={{ ...paragraph, fontSize: '13px', color: '#475569' }}>
              📎 Faktura č. {invoiceNumber} je přiložena k tomuto e-mailu (PDF).
              Uschovejte ji pro případnou reklamaci nebo daňové účely.
            </Text>
          )}
          {invoiceNumber && !invoiceAttached && (
            <Text style={{ ...paragraph, fontSize: '13px', color: '#475569' }}>
              🧾 Faktura č. {invoiceNumber} bude dostupná ke stažení ve vašem účtu po odeslání zásilky.
            </Text>
          )}

          <ItemsTable
            items={items}
            subtotalAmount={subtotalAmount}
            shippingAmount={shippingAmount}
            totalAmount={totalAmount}
          />

          <DeliveryBlock deliveryType={deliveryType} shippingAddress={shippingAddress} />

          <Button style={button} href={`${APP_URL}/account/orders`}>
            Sledovat objednávku
          </Button>

          <Text style={paragraph}>
            Máte otázky? Kontaktujte nás na{' '}
            <Link href="mailto:support@weedej.cz" style={link}>support@weedej.cz</Link>
          </Text>

          <Text style={paragraph}>S pozdravem,<br />Tým Weedej</Text>

          <Hr style={divider} />
          <Text style={footer}>
            Tento e-mail byl odeslán, protože jste zadali objednávku ve Weedej.
            Faktura je vystavena dle § 28 zákona č. 235/2004 Sb. (zákon o DPH).
          </Text>
        </Container>
      </Body>
    </Html>
  )
}

// ─── Template 2: Acknowledgement (ERP unavailable — no invoice yet) ───────────

/**
 * Mail #1 (fallback path): ERP unavailable — send acknowledgement without ESH number.
 * Invoice will be sent once ERP sync succeeds (cron job).
 */
export function OrderConfirmationAck({
  name,
  internalOrderId,
  items,
  subtotalAmount,
  shippingAmount,
  totalAmount,
  deliveryType,
  shippingAddress,
}: OrderConfirmationAckProps) {
  const refId = internalOrderId.slice(-8).toUpperCase()
  return (
    <Html lang="cs">
      <Head />
      <Preview>Přijali jsme vaši objednávku Weedej — děkujeme!</Preview>
      <Body style={body}>
        <Container style={container}>
          <Text style={logo}>Weedej</Text>

          <Text style={heading}>Přijali jsme vaši objednávku</Text>
          <Text style={paragraph}>Dobrý den, {name},</Text>
          <Text style={paragraph}>
            Děkujeme za vaši objednávku! Přijali jsme vaši platbu.
            Číslo objednávky a fakturu vám zašleme e-mailem co nejdříve.
          </Text>

          <Section style={orderBox}>
            <Text style={orderLabel}>Referenční číslo</Text>
            <Text style={orderNumberStyle}>#{refId}</Text>
          </Section>

          <Text style={{ ...paragraph, fontSize: '13px', color: '#475569' }}>
            📧 Fakturu obdržíte v samostatném e-mailu jakmile bude objednávka zpracována.
          </Text>

          <ItemsTable
            items={items}
            subtotalAmount={subtotalAmount}
            shippingAmount={shippingAmount}
            totalAmount={totalAmount}
          />

          <DeliveryBlock deliveryType={deliveryType} shippingAddress={shippingAddress} />

          <Button style={button} href={`${APP_URL}/account/orders`}>
            Zobrazit objednávky
          </Button>

          <Text style={paragraph}>
            Máte otázky? Kontaktujte nás na{' '}
            <Link href="mailto:support@weedej.cz" style={link}>support@weedej.cz</Link>
          </Text>

          <Text style={paragraph}>S pozdravem,<br />Tým Weedej</Text>

          <Hr style={divider} />
          <Text style={footer}>
            Tento e-mail byl odeslán, protože jste zadali objednávku ve Weedej.
          </Text>
        </Container>
      </Body>
    </Html>
  )
}

// Keep the original export for backward compatibility
export { OrderConfirmationWithInvoice as OrderConfirmation }

// ─── Styles ───────────────────────────────────────────────────────────────────

const body = {
  backgroundColor: '#F8F9FA',
  fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
}
const container = {
  backgroundColor: '#ffffff',
  border: '1px solid #DEE2E6',
  borderRadius: '12px',
  margin: '40px auto',
  padding: '40px',
  maxWidth: '560px',
}
const logo = { color: '#2E7D32', fontSize: '20px', fontWeight: '700', letterSpacing: '-0.5px', marginBottom: '24px' }
const heading = { color: '#1d1d1f', fontSize: '22px', fontWeight: '700', marginBottom: '8px' }
const paragraph = { color: '#6e6e73', fontSize: '15px', lineHeight: '1.6', marginBottom: '16px' }
const orderBox = { backgroundColor: '#F8F9FA', border: '1px solid #DEE2E6', borderRadius: '10px', padding: '16px 20px', marginBottom: '24px' }
const orderLabel = { color: '#6e6e73', fontSize: '11px', textTransform: 'uppercase' as const, letterSpacing: '0.05em', margin: '0 0 4px 0' }
const orderNumberStyle = { color: '#1d1d1f', fontSize: '18px', fontWeight: '700', fontFamily: 'monospace', margin: '0' }
const sectionTitle = { color: '#1d1d1f', fontSize: '14px', fontWeight: '600', marginBottom: '10px', marginTop: '8px' }
const itemRow = { marginBottom: '10px' }
const itemInfo = { verticalAlign: 'top' as const }
const itemName = { color: '#1d1d1f', fontSize: '14px', fontWeight: '500', margin: '0 0 2px 0' }
const itemVariant = { color: '#6e6e73', fontSize: '13px', margin: '0' }
const itemPrice = { verticalAlign: 'top' as const, textAlign: 'right' as const }
const itemPriceText = { color: '#1d1d1f', fontSize: '14px', margin: '0' }
const totalRow = { marginBottom: '4px' }
const totalLabel = { color: '#6e6e73', fontSize: '13px', margin: '0' }
const totalValue = { color: '#1d1d1f', fontSize: '13px', textAlign: 'right' as const, margin: '0' }
const grandTotalLabel = { color: '#1d1d1f', fontSize: '15px', fontWeight: '700', margin: '0' }
const grandTotalValue = { color: '#2E7D32', fontSize: '15px', fontWeight: '700', textAlign: 'right' as const, margin: '0' }
const button = { backgroundColor: '#2E7D32', borderRadius: '8px', color: '#ffffff', display: 'inline-block', fontWeight: '600', fontSize: '15px', padding: '14px 28px', textDecoration: 'none', marginBottom: '24px', marginTop: '8px' }
const divider = { borderColor: '#DEE2E6', marginTop: '16px', marginBottom: '16px' }
const link = { color: '#2E7D32' }
const footer = { color: '#aeaeb2', fontSize: '11px', lineHeight: '1.5' }
