...
export function OrderDelivered({ name, orderNumber, items, totalAmount }: OrderDeliveredProps) {
  return (
    <Html lang="cs">
      <Head />
      <Preview>Vaše objednávka {orderNumber} byla doručena — děkujeme!</Preview>
      <Body style={body}>
        <Container style={container}>
          <Text style={logo}>Weedej</Text>

          <Text style={heading}>Objednávka doručena</Text>
          <Text style={paragraph}>Dobrý den, {name},</Text>
          <Text style={paragraph}>
            Vaše objednávka {orderNumber} byla úspěšně doručena. Doufáme, že jste spokojeni
            s nákupem. Pokud máte jakékoliv otázky nebo připomínky, neváhejte nás kontaktovat.
          </Text>

          <Section style={deliveredBox}>
            <Text style={deliveredLabel}>Číslo objednávky</Text>
            <Text style={orderNumberStyle}>{orderNumber}</Text>
          </Section>

          <Text style={sectionTitle}>Doručené položky</Text>
          {items.map((item, i) => (
...
const deliveredBox = { backgroundColor: '#ECFDF5', border: '1px solid #6EE7B7', borderRadius: '10px', padding: '16px 20px', marginBottom: '24px' }
const deliveredLabel = { color: '#065F46', fontSize: '11px', textTransform: 'uppercase' as const, letterSpacing: '0.05em', margin: '0 0 4px 0' }
const orderNumberStyle = { color: '#064E3B', fontSize: '18px', fontWeight: '700', fontFamily: 'monospace', margin: '0' }
