...
export default function CartSummary({ totalPrice, itemCount, onCheckout, compact = false }: CartSummaryProps) {
  ...
  return (
    <div className={`bg-[#F8F9FA] border border-[#DEE2E6] rounded-2xl p-5 ${compact ? '' : 'sticky top-24'}`}>
      <h2 className="text-sm font-semibold text-[#1d1d1f] mb-3">Shrnutí objednávky</h2>

      <div className="flex items-center justify-between py-1.5">
        <span className="text-sm text-[#6e6e73]">Mezisoučet ({itemCount} {itemCount === 1 ? 'položka' : itemCount >= 2 && itemCount <= 4 ? 'položky' : 'položek'})</span>
      ...
      <div className="flex items-start justify-between py-1.5 border-t border-[#DEE2E6]">
        <div className="flex-1">
          <span className="text-sm text-[#6e6e73]">Doprava</span>
          {!shippingFree && (
        ...
        {shippingFree ? (
          <span className="text-sm font-medium text-[#2E7D32]">Zdarma</span>
        ) : (
